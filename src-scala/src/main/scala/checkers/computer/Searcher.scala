...
class Searcher(val moveGenerator: MoveGenerator,
  ...
  protected lazy val log: Logger = checkers.logger.computerPlayer
  ...
  def withMoveGenerator(newMoveGenerator: MoveGenerator): Searcher = {
    new Searcher(newMoveGenerator, moveExecutor, evaluator, drawLogic, performanceClock)
  }

  def withLogger(logger: Logger): Searcher = {
    new Searcher(moveGenerator, moveExecutor, evaluator, drawLogic, performanceClock) {
      override lazy val log: Logger = logger
    }
  }
